...
public class EnumDelegateMap
{
    ...
    private readonly Dictionary<Acts, Action<AbilityContext>> _myDelegatesMap = new();

    /// <summary>
    /// 내부 델리게이트 맵(_myDelegatesMap)에 대한 읽기 전용 뷰를 제공합니다.
    /// 외부에서는 이 맵의 내용을 읽을 수만 있고 변경할 수는 없습니다.
    /// </summary>
    public IReadOnlyDictionary<Acts, Action<AbilityContext>> GetActions => _myDelegatesMap;
    ...
    public void RemoveAction(Acts act, Action<AbilityContext> action = null)
    {
        ...
        {
            _myDelegatesMap.Remove(act); // 해당 Acts와 연결된 모든 델리게이트 제거
        }
    }
